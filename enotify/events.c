...
int CheckNewEvents(WPARAM wparam,LPARAM lparam)
{
	...
	if(dbei.eventType!=EVENTTYPE_MESSAGE && dbei.eventType!=EVENTTYPE_FILE && dbei.eventType!=EVENTTYPE_URL && dbei.eventType!=EVENTTYPE_AUTHREQUEST && dbei.eventType!=EVENTTYPE_ADDED)
		return 0;

	if(dbei.flags&DBEF_SENT)
		return 0;

	mode=DBGetContactSettingWord(NULL,MODULE,"ModeFlag",NOTIFY_MSG|NOTIFY_URL|NOTIFY_FILE|NOTIFY_ADDED|NOTIFY_AUTHREQ|DISMISS_AWAY|DISMISS_NA|DISMISS_DND);
	...
	if(dbei.eventType==EVENTTYPE_AUTHREQUEST && !(mode&NOTIFY_AUTHREQ))
		return 0;

	if(dbei.eventType==EVENTTYPE_ADDED && !(mode&NOTIFY_ADDED))
		return 0;

	status=(WORD)CallProtoService((const char *)CallService(MS_PROTO_GETCONTACTBASEPROTO,wparam,0),PS_GETSTATUS,0,0);
